...
public class DeleteBlock : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        brushManager = FindObjectOfType<BrushManager>();
    }
    ...
    void Update()
    {
        ...
        {
            DeleteBlocks(Vector3Int.FloorToInt(pointer.points[1]), triggerValue);
        }
    }

    //TODO: Dodaj typy pędzla
    void DeleteBlocks(Vector3Int position, float value)
    {
        int additional;
        if (value == 1)
            additional = 3;
                        ...
                        case 0:
                            if (brushManager.brushMode == 1)
                            {
                                ...
                                if (distance <= additional)
                                    world.SetBlock(i, j, k, new BlockAir());
                            }
                            else world.SetBlock(i, j, k, new BlockAir());
                            break;
                        case 1:
